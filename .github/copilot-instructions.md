...
- Use npm run commit for interactive assistant or VS Code extension vivaxy.vscode-conventional-commits
- Never edit CHANGELOG.md manually; it is auto-generated by commit-and-tag-version
- Follow the Release Process: run npm run release in release branch, create tag on main after merge
- Tags are created on main after merge, never on the release branch

## Conventional Commits and Tooling

All commits must follow Conventional Commits v1.0.0 specification. Format: <type>(<optional scope>): <description>. Valid types: feat, fix, docs, style, refactor, perf, test, build, ci, chore, revert.

Tooling stack:
- commitlint: Validates commit messages against Conventional Commits rules. Config: commitlint.config.js.
- husky: Git hooks manager. commit-msg hook runs commitlint automatically. Config: .husky/commit-msg.
- commitizen: Interactive commit assistant via npm run commit. Config: package.json > config.commitizen using cz-conventional-changelog adapter.
- commit-and-tag-version: Generates version bump + CHANGELOG.md + release commit. Config: .versionrc.json. Successor to standard-version.
- VS Code extension vivaxy.vscode-conventional-commits: Recommended in .vscode/extensions.json for visual commit assistance.

SemVer impact: feat → MINOR bump, fix/perf → PATCH bump, BREAKING CHANGE or ! → MAJOR bump, other types → no version bump.

## Semantic Versioning and Release Process

Project uses Semantic Versioning 2.0.0 (MAJOR.MINOR.PATCH). Version is auto-calculated by commit-and-tag-version.

Release scripts: npm run release:preview (dry-run preview without changes), npm run release (auto-calculate), npm run release:first (first release, CHANGELOG only), npm run release:major, npm run release:minor, npm run release:patch.

Release flow (Git Flow + commit-and-tag-version):
1. Preview version from develop: git checkout develop && npm run release:preview (shows calculated version without changes)
2. Create release/X.Y.Z branch from develop using calculated version
3. Run npm run release (updates package.json, generates CHANGELOG.md, creates chore(release): X.Y.Z commit)
4. Push branch, create PR to main, get approval, merge
5. On main: git tag -a vX.Y.Z -m "chore(release): X.Y.Z" then git push origin vX.Y.Z
6. Merge main back to develop
7. Delete release branch

Critical: Tag is NOT created by commit-and-tag-version (skip.tag: true in .versionrc.json). Tag must be created manually on main after merge because squash merge creates a new commit.

Hotfix release: Same process from main branch, typically npm run release:patch.

CHANGELOG.md: Auto-generated. Never edit manually. Visible types: Features (feat), Bug Fixes (fix), Performance (perf), Reverts (revert). Other types hidden.
